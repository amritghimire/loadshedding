...
class loadsheddingData:
    ...
    def update_dicti(self, dicti):
        with open(os.path.join(os.path.expanduser(r'~'), '.loadshedding', 'group'), "w") as text_file:
            text_file.write(str(dicti["group"]))
        with open(os.path.join(os.path.expanduser(r'~'), '.loadshedding', 'latest'), "w") as text_file:
    ...
    def refresh_data(self, dicti):
        ...
        if not (dicti["latest"][:10] == liness[1]):
            ...
            for i in range(1, 8):
                dts = self.getat(i)
                fln = os.path.join(os.path.expanduser(r'~'), '.loadshedding', 'dat' + str(i))
    ...
    def retrieve(self):
        group = open(os.path.join(os.path.expanduser(r'~'), '.loadshedding', 'group')).read()
        latest = open(os.path.join(os.path.expanduser(r'~'), '.loadshedding', 'latest')).read()
        data = open(os.path.join(os.path.expanduser(r'~'), '.loadshedding', 'data')).read()
        dicti = {"group": int(group), "latest": latest, "data": data}
        return dicti

    def perform_first_use(self):
        if os.path.exists(os.path.join(os.path.expanduser(r'~'), '.loadshedding')):
            return self.retrieve()
        else:
            os.makedirs(os.path.join(os.path.expanduser(r'~'), '.loadshedding'))
            self.update_dicti({"group": 7, "latest": "never", "data": "Not updated"})
            return self.refresh_data(None)

    def getDatFile(self, group):
        fln = os.path.join(os.path.expanduser(r"~"), ".loadshedding", 'dat' + str(group))
